...
一个运行在浏览器里的完整 Python 3.12 环境，无需安装，无需服务器，打开即用。
...
项目仍然保持 **单文件部署**，但内部 JavaScript 已重构为“事件驱动 + 依赖注入”的模块化架构：

- `EventBus`：模块间解耦通信（运行生命周期、标签生命周期、设置变更等）
- `StorageManager`：统一持久化（Tab 状态 + 防抖保存）
- `SettingsManager`：统一设置加载/保存
- `TabManager`：标签页状态管理
- `EditorEngine`：编辑器内容变更与状态同步
- `PyodideRunner`：Pyodide 运行时绑定与生命周期事件
- `AppCore.pwaManager`：Service Worker 注册与 PWA 安装流程

这种方式仍然是一个 `index.html`，但已经具备清晰的职责边界；后续新增功能可通过“新类 + EventBus 事件订阅”方式扩展，而无需继续堆全局函数。
